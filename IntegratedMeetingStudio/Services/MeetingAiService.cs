...
public sealed class MeetingAiService
{
    ...
    public async Task<string> GenerateTitleAsync(string summary, CancellationToken cancellationToken)
    {
        EnsureReady();
        if (string.IsNullOrWhiteSpace(summary)) return "새로운 회의록";

        var prompt = $"다음 회의 요약본을 바탕으로 핵심을 담은 1줄짜리 짧은 제목을 만들어줘. 응답은 오직 제목 텍스트만 출력해야 해.\n\n{summary}";
        
        var messages = new[] { new ChatRequestMessage("user", prompt) };
        try 
        {
            var title = await CreateChatCompletionAsync(messages, 50, cancellationToken, null);
            return string.IsNullOrWhiteSpace(title) ? "새로운 회의록" : title.Trim('"', ' ', '\n', '\r');
        }
        catch 
        {
            return "새로운 회의록";
    ...
    public async Task<List<IntegratedMeetingStudio.Models.SpeakerStat>> AnalyzeSpeakerStatsAsync(string transcript, CancellationToken cancellationToken)
    {
        EnsureReady();
        if (string.IsNullOrWhiteSpace(transcript)) return new List<IntegratedMeetingStudio.Models.SpeakerStat>();

        var prompt = $@"다음은 발언자가 구분된 회의록 원문이야. 각 화자별로 다음을 분석해서 JSON 배열로만 반환해줘.
형식: [{{""speaker"":""이름"",""speechCount"":0,""speechRatio"":0.0,""aiAnalysis"":""발언 성향 및 핵심 내용 요약""}}]
(speechRatio는 전체 발화 대비 퍼센트 또는 0~1 사이의 비율로 계산)
...
{transcript}";
        ...
        try 
        {
            var response = await CreateChatCompletionAsync(messages, 1500, cancellationToken, null);
            int startIdx = response.IndexOf('[');
            int endIdx = response.LastIndexOf(']');
            if (startIdx >= 0 && endIdx > startIdx)
            {
                var jsonStr = response.Substring(startIdx, endIdx - startIdx + 1);
                return JsonSerializer.Deserialize<List<IntegratedMeetingStudio.Models.SpeakerStat>>(jsonStr) ?? new List<IntegratedMeetingStudio.Models.SpeakerStat>();
            }
        }
        catch { }
        return new List<IntegratedMeetingStudio.Models.SpeakerStat>();
    }

    public async Task<List<IntegratedMeetingStudio.Models.NextAgendaItem>> SuggestNextAgendaAsync(string transcriptOrSummary, CancellationToken cancellationToken)
    {
        EnsureReady();
        if (string.IsNullOrWhiteSpace(transcriptOrSummary)) return new List<IntegratedMeetingStudio.Models.NextAgendaItem>();

        var prompt = $@"다음 회의 내용을 바탕으로 다음 회의에서 다루면 좋을 추천 안건(Agenda)들을 도출해줘. 미결 과제나 후속 논의가 필요한 부분 중심.
응답은 반드시 JSON 배열로만 줘.
형식: [{{""title"":""안건 제목"",""description"":""왜 이 안건을 다뤄야 하는지 설명""}}]
...
{transcriptOrSummary}";
        ...
        try 
        {
            ...
            {
                var jsonStr = response.Substring(startIdx, endIdx - startIdx + 1);
                return JsonSerializer.Deserialize<List<IntegratedMeetingStudio.Models.NextAgendaItem>>(jsonStr) ?? new List<IntegratedMeetingStudio.Models.NextAgendaItem>();
            }
        }
        catch { }
        return new List<IntegratedMeetingStudio.Models.NextAgendaItem>();
    }

    public List<IntegratedMeetingStudio.Models.TimelineItem> ParseTimeline(string transcript)
    {
        var result = new List<IntegratedMeetingStudio.Models.TimelineItem>();
        if (string.IsNullOrWhiteSpace(transcript)) return result;

        var lines = transcript.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
        var speakerRegex = new Regex(@"^([가-힣a-zA-Z0-9_\s]+)\s*[:：](.+)$");
        ...
        foreach (var line in lines)
        {
            ...
            if (match.Success)
            {
                result.Add(new IntegratedMeetingStudio.Models.TimelineItem
                {
            ...
            {
                // Append to previous speaker if it's a multiline speech
                result[^1].Text += " " + line.Trim();
            }
            else
            {
                // No speaker defined yet, maybe just text
                result.Add(new IntegratedMeetingStudio.Models.TimelineItem
                {
                    Speaker = "알 수 없음",
                    Text = line.Trim(),
