...
namespace IntegratedMeetingStudio.Controls
{
    public partial class KanbanBoardView : UserControl
    {
        ...
        public KanbanBoardView()
        {
            ...
            this.pnlHeader.Controls.Add(btnDeleteSelected);
            this.pnlHeader.Controls.Add(btnDeleteAll);
        }
        ...
        private void BtnDeleteSelected_Click(object? sender, EventArgs e)
        {
            ...
            var cardsToRemove = new List<TaskItem>();
            foreach (Control pnl in new[] { pnlTodo, pnlInProgress, pnlDone })
            {
                foreach (Control c in pnl.Controls)
                {
                    if (c is TaskCard card && card.IsSelected)
                    {
                        cardsToRemove.Add(card.Task);
                    }
                }
            }
            ...
            {
                if (MessageBox.Show($"선택한 {cardsToRemove.Count}개의 업무를 삭제하시겠습니까?", "선택 삭제", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    foreach (var task in cardsToRemove)
                    {
                        _projectData.Tasks.Remove(task);
                    }
            ...
            {
                MessageBox.Show("삭제할 업무를 먼저 체크박스로 선택해주세요.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private void BtnDeleteAll_Click(object? sender, EventArgs e)
        {
            if (_projectData == null || _projectData.Tasks.Count == 0) return;

            if (MessageBox.Show("모든 업무를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.", "전체 삭제", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
            {
                _projectData.Tasks.Clear();
                DataManager.SaveData(_projectData);
                RefreshBoard();
            }
        }
