...
public sealed class MeetingStorageService
{
    ...
    public MeetingRecord Save(string audioPath, string transcript, string summary)
    {
    ...
    private static string CreateMarkdown(MeetingRecord record)
    {
        var summary = string.IsNullOrWhiteSpace(record.Summary)
            ? "AI 회의록은 아직 생성되지 않았습니다. STT 원문만 저장된 기록입니다."
            : record.Summary;

        return $"""
        # 회의록 자동 정리 결과

        - 생성 일시: {record.CreatedAt:yyyy-MM-dd HH:mm:ss}
        - 원본 파일: {record.AudioPath}
